...
#' @export
obtain_mortality_data <- function(pop_data, current_date) {
  ...
  ## Output ----
  # -------------------------- -
  mortality_data <- data.frame(ind_id = ind_id, 
                               time_of_death = 
                                 pmin(pop_data$time_of_natural_death,
                                    pop_data$time_of_death, na.rm = T),
                               time = current_date)
  mortality_data <- subset(mortality_data, !is.na(time_of_death))
  mortality_data <- subset(mortality_data, time_of_death < current_date)
